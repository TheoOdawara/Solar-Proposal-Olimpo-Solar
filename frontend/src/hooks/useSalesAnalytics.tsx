...
export const useSalesAnalytics = () => {
  ...
  const loadSalesData = useCallback(async () => {
    ...
    try {
      ...
      // Build the query builder first, apply modifiers, then call select()
      let builder = apiClient.from('proposals');

      // Apply ordering before selecting (shim executes fetch in select())
      ...
      if (dateRange.to) {
        builder = builder.lte('created_at', dateRange.to.toISOString());
      }

      // Apply seller filter
      if (selectedSeller !== 'all') {
        builder = builder.eq('seller_id', selectedSeller);
      }

      const resp = await builder.select(`
        id,
        seller_name,
        seller_id,
        client_name,
