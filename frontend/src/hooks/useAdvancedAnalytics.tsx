...
export const useAdvancedAnalytics = () => {
  ...
  const generateNotifications = useCallback((data: ProposalAnalytics[]) => {
    ...
    const decliningSellersByFiftyPercent = Object.entries(sellerStats).filter(([, stats]) => 
  ...
  const loadProposals = useCallback(async () => {
    try {
      // call order before select so the shim can build the query correctly
      const resp = await apiClient
        .from('proposals')
        .order('created_at', { ascending: false })
        .select('*') as ApiResponse<ProposalAnalytics[]>;
      ...
      generateDailyStats(Array.isArray(rows) ? rows : []);
      generateNotifications(Array.isArray(rows) ? rows : []);
    } catch (error: unknown) {
